  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-[1000] transition-all duration-500 px-[6%] py-4 flex items-center justify-between ${isScrolled ? 'bg-white shadow-lg' : 'bg-transparent'}`}>
        ...
        <ul className="flex items-center gap-10">
          {navLinks.map((link) => (
            <li key={link.path}>
              <Link
                to={link.path}
                className={`text-[12px] uppercase tracking-[0.3em] font-black transition-all hover:text-[#7A2318] relative group ${
      ...
      <div className="flex items-center gap-4">
        <Link
          to="/contact"
          className={`hidden md:block bg-[#7A2318] text-white px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] hover:bg-white hover:text-[#7A2318] transition-all shadow-xl hover:-translate-y-0.5 active:scale-95`}
        >
          Enquire Now
        </Link>
